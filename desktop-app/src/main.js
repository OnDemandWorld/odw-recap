...
async function viewMeetingDetails(id) {
    ...
    <div class="detail-content">
      <div class="detail-section">
        ...
        <div class="detail-actions">
          ...
          <button id="extract-structured-btn" class="btn-secondary">Extract Action Items & Decisions</button>
        </div>
      </div>
      <div class="detail-section">
        <h3>Transcript</h3>
      ...
      <div class="detail-section">
        <h3>Action Items</h3>
        <div id="action-items-content"><p class="placeholder-text">Loading...</p></div>
      </div>
      <div class="detail-section">
        <h3>Decisions</h3>
        <div id="decisions-content"><p class="placeholder-text">Loading...</p></div>
      </div>
    </div>
  `;
  ...
  document.querySelector("#extract-structured-btn").addEventListener("click", extractStructuredData);

  await Promise.all([loadTranscript(id), loadSummary(id), loadActionItems(id), loadDecisions(id)]);
...
async function loadActionItems(meetingId) {
  const el = document.querySelector("#action-items-content");
  try {
    const items = await invoke("list_action_items", { meetingId });
    if (!items || items.length === 0) {
      el.innerHTML = '<p class="placeholder-text">No action items yet. Click "Extract Action Items & Decisions" to generate them.</p>';
      return;
    }
    el.innerHTML = items
      .map((item) => `
        <div class="action-item" data-id="${item.id}">
          <div class="action-item-header">
            <input type="checkbox" class="action-item-checkbox" data-id="${item.id}" ${item.status === 'done' ? 'checked' : ''}>
            <span class="action-item-description ${item.status === 'done' ? 'done' : ''}">${escapeHtml(item.description)}</span>
            <button class="btn-small delete-action-item-btn" data-id="${item.id}">Delete</button>
          </div>
          <div class="action-item-meta">
            ${item.assignee ? `<span><strong>Assignee:</strong> ${escapeHtml(item.assignee)}</span>` : ''}
            ${item.deadline ? `<span><strong>Deadline:</strong> ${escapeHtml(item.deadline)}</span>` : ''}
          </div>
        </div>
      `)
      .join("");
    ...
    document.querySelectorAll(".action-item-checkbox").forEach((checkbox) => {
      checkbox.addEventListener("change", async (e) => {
        ...
        try {
          await invoke("update_action_item_status", { id, status: newStatus });
          await loadActionItems(meetingId);
        } catch (error) {
          setStatus(`Failed to update action item: ${error}`);
          await loadActionItems(meetingId);
        }
      });
    });

    document.querySelectorAll(".delete-action-item-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        ...
        try {
          ...
          setStatus(`Failed to delete action item: ${error}`);
        }
      });
    });
  } catch (error) {
    el.innerHTML = `<p class="placeholder-text">Failed to load action items: ${escapeHtml(String(error))}</p>`;
  }
}

async function loadDecisions(meetingId) {
  const el = document.querySelector("#decisions-content");
  try {
    const decisions = await invoke("list_decisions", { meetingId });
    if (!decisions || decisions.length === 0) {
      el.innerHTML = '<p class="placeholder-text">No decisions yet. Click "Extract Action Items & Decisions" to generate them.</p>';
      return;
    }
    el.innerHTML = decisions
      .map((dec) => `
        <div class="decision" data-id="${dec.id}">
          <div class="decision-header">
            <span class="decision-description">${escapeHtml(dec.description)}</span>
            <button class="btn-small delete-decision-btn" data-id="${dec.id}">Delete</button>
          </div>
          ${dec.context ? `<div class="decision-context"><strong>Context:</strong> ${escapeHtml(dec.context)}</div>` : ''}
          ${dec.participants && dec.participants.length > 0 ? `<div class="decision-participants"><strong>Participants:</strong> ${dec.participants.map(escapeHtml).join(", ")}</div>` : ''}
        </div>
      `)
      .join("");

    // Add event listeners
    document.querySelectorAll(".delete-decision-btn").forEach((btn) => {
      btn.addEventListener("click", async (e) => {
        const id = e.target.dataset.id;
        if (!confirm("Delete this decision?")) return;
        try {
          await invoke("delete_decision", { id });
          await loadDecisions(meetingId);
        } catch (error) {
          setStatus(`Failed to delete decision: ${error}`);
        }
      });
    });
  } catch (error) {
    el.innerHTML = `<p class="placeholder-text">Failed to load decisions: ${escapeHtml(String(error))}</p>`;
  }
}

async function extractStructuredData() {
  if (!currentMeetingId) return;
  const btn = document.querySelector("#extract-structured-btn");
  btn.disabled = true;
  setStatus("Extracting action items and decisions...");
  try {
    const [actionItems, decisions] = await invoke("extract_action_items_and_decisions", {
      meetingId: currentMeetingId,
    });
    setStatus(`Extracted ${actionItems.length} action item(s) and ${decisions.length} decision(s)`);
    await Promise.all([loadActionItems(currentMeetingId), loadDecisions(currentMeetingId)]);
  } catch (error) {
